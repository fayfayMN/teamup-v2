...
def roster_csv(pool: list) -> str:
    ...
    buf = io.StringIO()
    w = csv.writer(buf)
    w.writerow(["Name", "Level", "Major", "School", "Roles", "Skills",
                "Wants to learn", "Availability", "Hours/week", "Commitment"])
    for p in pool:
        w.writerow([
            p.name, getattr(p, "level", ""), getattr(p, "major", ""),
            getattr(p, "school", ""), "; ".join(sorted(p.roles())), "; ".join(p.skills),
...
def teams_csv(teams: list) -> str:
    ...
    buf = io.StringIO()
    w = csv.writer(buf)
    w.writerow(["Team", "Division", "Member", "Level", "Major", "School", "Roles",
                "Commitment", "Availability", "Team covered roles",
                "Team missing roles", "Schedule cohesion"])
    for i, t in enumerate(teams, 1):
        ...
        for m in t["members"]:
            w.writerow([
                f"Team {i}", division, m.name, getattr(m, "level", ""),
                getattr(m, "major", ""), getattr(m, "school", ""),
                "; ".join(sorted(m.roles())),
